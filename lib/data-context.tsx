...
export function DataProvider({ children }: { children: ReactNode }) {
  // Initialize with consistent default values for SSR
  ...
  const login = async (email: string, password: string): Promise<boolean> => {
    // Demo credentials with proper authentication
    const demoUsers = [
      {
        id: "emp-001",
        email: "employee@oneflow.com",
        password: "12345",
        name: "John Smith",
      ...
      },
      {
        ...
        role: "sales_finance" as UserRole,
        companyId: "demo-company"
      }
    ]
    ...
    // Fallback to stored users in localStorage
    const storedUsers = localStorage.getItem("users")
    if (storedUsers) {
      const allUsers: User[] = JSON.parse(storedUsers)
      const user = allUsers.find((u) => u.email === email)
      
      // For stored users, we'll accept any password since we don't store passwords securely
      if (user) {
        setCurrentUser(user)

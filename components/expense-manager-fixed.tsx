...
export function ExpenseManager({ 
  ...
  const stats = {
    ...
    totalValue: filteredExpenses.reduce((sum, exp) => sum + exp.amount, 0),
    approvedExpenses: filteredExpenses.filter(exp => exp.status === "approved").length,
    pendingExpenses: filteredExpenses.filter(exp => exp.status === "pending").length,
    billableValue: filteredExpenses
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-4">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage all expenses across projects
          </p>
        </div>
